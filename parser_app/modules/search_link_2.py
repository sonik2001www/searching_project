...
def sum_size(ls):
    ...
    for item in ls:
        size_str = item[1]  # Отримуємо рядок розміру файлу
        if size_str:
            size_str = size_str[:-3]  # Видаляємо ' KB' з рядка
            try:
                size_bytes = float(size_str) * 1024  # Конвертуємо кілобайти у байти
                total_size_bytes += size_bytes  # Додаємо розмір файлу до загального розміру
            except ValueError:
                print(f"Помилка конвертації розміру у байти для рядка: {size_str}")
        else:
            print(f"Невідомий формат розміру для рядка: {size_str}")

    total_size_formatted = format_size(total_size_bytes)  # Конвертуємо загальний розмір у відформатований рядок
